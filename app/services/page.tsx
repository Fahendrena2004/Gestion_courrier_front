...
import { Briefcase, Plus, Pencil, Trash2, Search, X, Loader2, Save } from 'lucide-react';
...
export default function ServicesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload = { name: formName, description: formDescription };
          ...
          {filtered.map((service) => (
            ...
            >
              <div className="space-y-3">
                <div className="font-bold text-lg text-slate-100 truncate">{service.name}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-md p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold text-white mb-6">
              {currentService ? 'Modifier le Service' : 'Nouveau Service'}
            </h3>
